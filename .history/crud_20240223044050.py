...
def get_favs(db: Session):
    favs = db.query(models.fav).all()
    return {"total": len(favs), "data": favs}

def create_fav(db: Session, fav: schemas.favCreate):
    new_fav = models.fav(**fav.dict())
    db.add(new_fav)
    db.commit()
    db.refresh(new_fav)
    return {"ID of the created fav": new_fav.id}

def get_fav(db: Session, fav_id: int):
    return db.query(models.fav).filter(models.fav.id==fav_id).first()

def update_fav(db: Session, fav_id: int, fav: schemas.favCreate):
    fav_data = get_fav(db, fav_id)
    if not fav_data:
        raise HTTPException(status_code=404, detail="fav not found")
    for key, value in fav.dict().items():
        setattr(fav_data,key,value)
    db.commit()
    return {"Message":f"Updated {fav_id} Successfully"}

def delete_fav(db: Session, fav_id: int):
    data = get_fav(db, fav_id)
    if not data:
        raise HTTPException(status_code=404, detail="fav not found")
    db.delete(data)
    db.commit()
    return {"Message":f"Deleted {fav_id} Successfully"}
