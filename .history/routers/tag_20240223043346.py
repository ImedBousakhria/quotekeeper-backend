...
async def create_quote(quote:schemas.TagCreate, db:Session=Depends(get_db)):
    return crud.create_quote(db, quote)

@routerTag.put('/update_quote')
async def update_quote(quote_id:int, quote_body: schemas.TagCreate, db:Session=Depends(get_db)):
