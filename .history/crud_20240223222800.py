...
def bookmark_quote(db: Session, quote_id:int):
    db_quote = db.query(models.Quote).filter(models.Quote.id == id_Quote).one_or_none()
    if db_quote is None:
        return {"erreur":"Quote non existante"}

    setattr(db_quote, "verified", True)

    db.commit()
    db.refresh(db_quote)
    return {"success":f"Quote '{db_quote.id}' has been approved"}
